...
from pico.StepMoterB import StepMoterB
...
# GPIO定義
P_MODE_SW = 17               # モード切替スイッチ（GPIO.IN, PULL_DOWN）
P_READ_SW = 18               # リロード起動スイッチ（GPIO.IN, PULL_DOWN）
P_STEP_MT = [10, 11, 12, 13] # エレベーター用ステップモーター（I2C）
P_STEP_MT_B = [10, 11, 12] # エレベーター用ステップモーター（I2C）
P_TURN_SV = 1                # ターンテーブル用サーボ（I2C）
P_PUSH_SV = 0                # 押し出し機用サーボ（I2C）
P_EXEC_LT = 16               # 動作中LED（GPIO。OUT）
STEPMOTOTER_IS_B = False     # バイボーラ式ステップモーター利用
...
if STEPMOTOTER_IS_B:
    jenga_table.setElevetorStepMotor(StepMoterB(P_STEP_MT_B[0], P_STEP_MT_B[1], P_STEP_MT_B[2]))
else:
    jenga_table.setElevetorStepMotor(StepMoter28BYJ48(P_STEP_MT[0], P_STEP_MT[1], P_STEP_MT[2], P_STEP_MT[3]))
jenga_table.setExecLed(Led(P_EXEC_LT))
# ジェンガテーブル微調整
jenga_table.setTurnTableServoRange(3, 153)
...
# メインループ:start
    if mode_switch.value() == 1:
        ...
        jenga_table.tableMove(int(v_turn))
        jenga_table.pushMove(int(v_push))
    else:
        # マニュアルモードスイッチOFF
